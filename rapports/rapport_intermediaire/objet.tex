...
\node[initial, state] (A) at (0,0) {Départ};
\node[state] (B) at (5,0)  {A};
\node[state] (C) at (5,-6) {B};
\node[state] (D) at (8,-4) {C};
\node[state, accepting, align=center] (E) at (0,-11) {Throw I/O \\ Exception};
...
\draw[->] (B) [left] -- node [near begin ,align=center, above, sloped]  {Buffer[1] != 0x34 \&\& Buffer[1] != 0x38} (E);
...
\draw[->] (D) [right] --  node [align=center]  {} (F);
\draw[->] (D) [right] |- node [near start, align=center, rotate=90, below]  {Buffer[2] != 0 \&\& Buffer[3] != 0}  node [near start, align=center, rotate=90, above] {balance < delMont} (E);
