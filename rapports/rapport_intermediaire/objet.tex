...
\paragraph{}
Une fois le graphe extrait, nous souhaitons créer un automate représentant le fonctionnement de l'application, en voici un exemple en lien avec le graphe précédant :
...
\node[state] (A) at (0,0) {Départ};
\node[state] (B) at (5,0)  {A};
\node[state] (C) at (5,-6) {B};
\node[state] (D) at (8,-4) {C};
\node[state, accepting, align=center] (E) at (0,-8) {Throw I/O \\ Exception};
\node[state, accepting] (F) at (6,-4) {Fin};


\draw[->] (A)[left]  -- node [align=center, above]  {Buffer[0] == 0x80} (B);
\draw[->] (A)[above] -- node[align=center, rotate=90]{Buffer[0] != 0x80} (E);
\draw[->] (B) [left] -- node [align=center, above, sloped]  {Buffer[1] != 0x34 \&\& Buffer[1] != 0x38} (E);
\draw[->] (B) [right]  -- node [align=end, above, rotate=90]  {Buffer[1] == 0x34} (C);
\draw[->] (B) [right] -| node [near end , align=center, below, rotate=90]  {Buffer[1] == 0x38}    (D);
\draw[->] (C) [loop below] |- node [align=center]  {Buffer[2] != 0 \&\& Buffer[3] != 0 } (E);
\draw[->] (C) [loop below] -| node [align=center]  {} (F);
\draw[->] (D) [right] --  (F);
\draw[->] (D) [right] |-   node [near start, align=center, rotate=90, below] {Balance < delMont}  (E);
\end{tikzpicture}
\caption{Exemple d'automate à générer}
\end{figure}
...
\paragraph{}
L’abstraction de code n’est pas nouveau dans le domaine du développement d’application. Il existe déjà de nombreux outils pour différents langages. Par exemple pour le Java il existe Bandera qui effectue tout un travail d’analyse. Cependant ce principe n’a jamais été appliqué à l’API de JavaCard. 

\paragraph{}
Notre travail consistera avant tout à utiliser des techniques, voir des outils, déjà existants mais en les adaptant pour qu’ils puissent analyser une applet JavaCard.

C’est dans ce but que nous allons vous présenter un état de l’art qui représente une importante partie de la suite de ce rapport.
